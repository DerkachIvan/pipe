class FluidSlot{
    ...
    TryInsert(fluid: Fluid | null, amount: number): boolean {
        ...
        if(this.constFluid){
            ...
            if(this.fluid.id !== fluid.id) return false;
        } 
        
        if(this.fluid === null) {
            if(amount > this.capacity) return false;
        ...
        if(this.fluid.quantity + amount > this.capacity) return false;

        this.fluid.quantity += amount;
        return true;
    }

    TryGet(amount = 0, fluid: Fluid | null = null): {fluid: Fluid | null, amount: number} {
        if(amount <= 0){
            if(this.fluid === null){
        ...
        if(this.fluid === null) return fluidGroup;

        if(this.fluid.id !== fluid.id) return fluidGroup;
