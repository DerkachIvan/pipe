...
class PipeSystem {
    ...
    canTransferFluid(sourcePipe, targetPipe) {
        if (!sourcePipe || !targetPipe)
            return false;
        if (!isFinite(targetPipe.flowLevel) || targetPipe.flowLevel <= sourcePipe.flowLevel)
            return false;
    ...
    bfs() {
        ...
        while (queue.length) {
            let p = queue.shift();
            if (p.flowLevel >= this.maxFlowLevel)
                continue;
    ...
    flowStep() {
        ...
        for (let p of this.pipes) {
            ...
            let valid = [];
            for (let n of p.getNeighborsPipes()) {
                if (!isFinite(n.flowLevel))
                    continue;
                if (!this.canTransferFluid(p, n))
                    continue;
        ...
        for (let p of this.pipes) {
            for (let n of p.getNeighborsPipes()) {
                if (n.flowLevel !== p.flowLevel)
                    continue;
                if (n.fluidType !== "empty" && n.fluidType !== p.fluidType)
                    continue;
