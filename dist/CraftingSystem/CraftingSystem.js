...
class CraftingSystem {
    ...
    static Finish(machine) {
        ...
        machine.output.TryInsert(recipe.output.product, recipe.output.amount);
    ...
    static ConsumeIngredients(machine, recipe) {
        const inputs = Array.isArray(recipe.input)
            ...
            : [recipe.input];
        for (const input of inputs) {
            const result = machine.input.TryGet(input.amount, input.material);
...
//# sourceMappingURL=CraftingSystem.js.map